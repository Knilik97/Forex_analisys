...
# --- НАСТРОЙКИ ---
SEQ_LEN = 240            # Сколько прошлых свечей модель будет "помнить"
BATCH_SIZE = 64
MODEL_NAME = "model_v7.1_EURUSD.h5" # Новое имя для новой модели
# ------------------
...
def main():
    print("🚀 Загрузка данных для V7.1...")

    INPUT_FILE = "/content/ready_for_training_EURUSD_H1.csv"
    ...
    model.compile(loss='mse', optimizer=opt) # Можно добавить metrics=['mae'] для графика точности
    ...
    print("🧠 Начало обучения модели V7.1...")
    ...
    # --- НОВОЕ: Сохранение истории и финальная оценка ---

    # 1. Оценка на тестовых данных (финальный результат)
    test_loss = model.evaluate(X_test, y_test, verbose=0)
    print(f"⚖️ Финальный loss (MSE) на тестовых данных: {test_loss:.6f}")

    # 2. Сохранение истории обучения для графиков в Colab
    import json
    with open('history_v7.1_EURUSD.json', 'w') as f:
        json.dump(history.history, f)

    # 3. Сохранение масштабировщиков и модели (гарантированно)
    joblib.dump(scaler_y, 'scaler_price_v7.1_EURUSD.pkl')
    joblib.dump(scaler_X, 'scaler_features_v7.1_EURUSD.pkl')
