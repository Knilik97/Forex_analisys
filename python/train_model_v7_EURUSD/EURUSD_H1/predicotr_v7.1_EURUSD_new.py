...
# --- НАСТРОЙКИ (ВАЖНО: УКАЖИТЕ ПРАВИЛЬНЫЕ ПУТИ) ---
SYMBOL = "EURUSD"
...
NUM_CANDLES_TO_DOWNLOAD = 300 # Скачиваем с запасом, чтобы хватило на расчет индикаторов

# ПУТИ К ФАЙЛАМ (ИЗ ВАШЕГО СКРИПТА ОБУЧЕНИЯ)
MODEL_FILE = "python/train_model_v7_EURUSD/EURUSD_H1/model_regression_new_EURUSD_H1.h5" # Модель из train.py
SCALER_FEATURES_FILE = "python/train_model_v7_EURUSD\EURUSD_H1/scaler_features_regression_new.pkl" # Скейлер признаков из train.py
SCALER_PRICE_FILE = "python/train_model_v7_EURUSD/EURUSD_H1/scaler_price_regression_new.pkl" # Скейлер цены из train.py
...
def main():
    ...
    # 2. СОЗДАНИЕ DATAFRAME И РАСЧЕТ ИНДИКАТОРОВ (ТОЧНО КАК В ПЕРВОМ СКРИПТЕ)
    ...
    # Индикаторы (копия из вашего первого кода)
    df['EMA_20'] = close_prices.ewm(span=20, adjust=False).mean()
    df['EMA_50'] = close_prices.ewm(span=50, adjust=False).mean()

    # RSI (встроенный метод ta был заменен на ручной расчет для автономности)
    ...
    # ATR (встроенный метод ta заменен на ручной расчет True Range)
    ...
    # Список признаков ДОЛЖЕН СОВПАДАТЬ с тем, что был в train.py
    features_cols = [
    ...
    try:
        ...
        print("Убедитесь, что файлы модели и скейлеров находятся в той же папке или указан верный путь.")
        return
